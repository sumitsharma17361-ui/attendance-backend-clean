...
const userSchema = new mongoose.Schema({
  ...
  role: { type: String, default: 'student' } // 'student' or 'faculty'
}, { timestamps: true });
...
app.post('/api/attendance/mark', async (req, res) => {
  try {
    ...
    // --- GEO-FENCING LOGIC (College Coordinates Setup) ---
    // Example Coordinates (Set to your Campus Location)
    ...
    if (distance > MAX_ALLOWED_DISTANCE_KM) {
      return res.status(400).json({ 
        error: `Attendance Failed: You are outside the College Campus! (${(distance * 1000).toFixed(0)} meters away)` 
      });
    }
...
// 4. Get Student Attendance History API
